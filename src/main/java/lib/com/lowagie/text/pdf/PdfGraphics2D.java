...
import lib.com.lowagie.text.pdf.internal.PolylineShape;
...
*/
    ...
    private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
        ...
        try {
            lib.com.lowagie.text.Image image = null;
            if(!convertImagesToJPEG){
                image = lib.com.lowagie.text.Image.getInstance(img, bgColor);
            }
            else{
                ...
                scaled.flush();
                scaled = null;
                image = lib.com.lowagie.text.Image.getInstance(baos.toByteArray());
                
            }
            if (mask!=null) {
                lib.com.lowagie.text.Image msk = lib.com.lowagie.text.Image.getInstance(mask, null, true);
                msk.makeMask();
    ...
    private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
        if (paint instanceof Color) {
            ...
            try {
                ...
                lib.com.lowagie.text.Image image = lib.com.lowagie.text.Image.getInstance(img, null);
